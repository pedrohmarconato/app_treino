...
async function loadCadastroSystem() {
    ...
    try {
        ...
        console.log('[cadastroSystem] ✅ userRegistrationService carregado:', !!registrationModule.cadastrarNovoUsuario);
        ...
        console.log('[cadastroSystem] ✅ userValidationService carregado:', !!validationModule.validateUserData);
        ...
        if (resultado) {
            ...
            try {
                ...
                if (resultadoSalvar.success) {
                    ...
                    console.error('[abrirQuestionarioUsuario] ❌ Erro ao salvar questionário:', resultadoSalvar.error);
                }
            } catch (saveError) {
...
window.abrirModalCadastro = async function(e) {
    ...
    try {
        ...
        console.log('[abrirModalCadastro] 🔍 CadastroUsuarioModal disponível:', !!window.CadastroSystem.CadastroUsuarioModal);
        console.log('[abrirModalCadastro] 🔍 Corpo do documento:', !!document.body);
        console.log('[abrirModalCadastro] 🔍 Modal existente no DOM:', !!document.getElementById('cadastro-modal-overlay'));
        ...
        if (resultado) {
            ...
            try {
                ...
                if (questionarioResultado) {
                    ...
                    if (window.showNotification) {
                        window.showNotification('Cadastro concluído! Você pode preencher seu perfil depois.', 'info');
                    }
                }
            } catch (questionarioError) {
                console.error('[abrirModalCadastro] ❌ Erro no questionário:', questionarioError);
                if (window.showNotification) {
                    window.showNotification('Usuário cadastrado, mas questionário não pôde ser aberto.', 'warning');
                }
            }
            return null;
        }
...
window.testarSistemaCadastro = async function() {
    ...
    try {
        ...
        console.log('[testarSistemaCadastro] 🎯 Funções disponíveis:', Object.keys(window.CadastroSystem));
...
loadCadastroSystem().then(success => {
    if (success) {
        ...
        console.log('[cadastroSystem] 📋 Use abrirQuestionarioUsuario(usuario) para abrir questionário');
