...
document.addEventListener('DOMContentLoaded', () => {
  const container =
    document.getElementById('treino-options') || document.getElementById('opcoes-treino-container');
  if (container) {
...
function validarPlanejamento() {
  const btnSalvar =
    document.querySelector('.btn-save') || document.getElementById('confirm-plan-btn');
  ...
  for (const diaKey in planejamentoAtual) {
    if (planejamentoAtual.hasOwnProperty(diaKey) && planejamentoAtual[diaKey]) {
      ...
      if (treinoDoDia.categoria === 'muscular') {
        tiposMuscularesNoPlano[treinoDoDia.tipo] =
          (tiposMuscularesNoPlano[treinoDoDia.tipo] || 0) + 1;
      }
    }
  }
    ...
    const option = criarOpcaoTreino(
      {
        ...
        tipo: treino.nome,
        categoria:
          treino.nome === 'Folga' ? 'folga' : treino.nome === 'Cardio' ? 'cardio' : 'muscular',
      },
      dia
    );
    ...
    if (!resultado.success) {
      if (window.showNotification) {
        window.showNotification(
          '❌ Erro ao salvar no Supabase: ' + (resultado.error || 'Erro desconhecido'),
          'error'
        );
      }
