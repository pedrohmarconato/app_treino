...
export class WorkoutScreen extends BaseComponent {
  ...
  cacheElements() {
    this.elements = {
      ...
      // Timer
      timerDisplay:
        this.querySelector('#workout-timer-display') || this.querySelector('#workout-timer'),
  ...
  async initializeSubsystems() {
    ...
    const [{ ExerciseManager }, { TimerManager }, { PersistenceManager }] = await Promise.all([
  ...
  setupEventListeners() {
    ...
    const unsubscribeExerciseComplete = eventBus.on(SystemEvents.EXERCISE_COMPLETED, (data) =>
      this.onExerciseCompleted(data)
    );

    const unsubscribeSeriesComplete = eventBus.on(SystemEvents.SERIES_COMPLETED, (data) =>
      this.onSeriesCompleted(data)
    );

    // Guardar unsubscribe functions para cleanup
    this.eventUnsubscribers = [unsubscribeExerciseComplete, unsubscribeSeriesComplete];
