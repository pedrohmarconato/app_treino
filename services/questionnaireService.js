...
export async function salvarQuestionario(questionnaireData) {
    try {
        ...
        if (errorVerificar && errorVerificar.code !== 'PGRST116') {
            console.error('[QuestionnaireService] ❌ Erro ao verificar questionário existente:', errorVerificar);
...
export async function verificarQuestionarioExistente(userId) {
    try {
        ...
        console.log('[QuestionnaireService] ✅ Questionário existe para usuário:', userId, 'desde', data.created_at);
        return true;
...
export function validarDadosQuestionario(data) {
    ...
    if (!data.objetivo || !['hipertrofia', 'emagrecimento', 'forca', 'resistencia', 'condicionamento'].includes(data.objetivo)) {
    ...
    if (!data.tempo_por_treino || !['30', '45', '60', '90'].includes(data.tempo_por_treino.toString())) {
