...
export class WorkoutProtocolService {
  ...
  static async carregarTreinoParaExecucao(userId) {
    try {
      ...
      const protocoloId =
        planejamentoHoje.protocolo_treinamento_id || planoUsuario.protocolo_treinamento_id;
      ...
      console.log(
        `[WorkoutProtocol] ✅ Treino carregado: ${exerciciosCompletos.length} exercícios`
      );
  ...
  static async buscarTreinoDeHoje(userId, semanaProtocolo = null) {
    try {
      ...
      console.log(
        `[WorkoutProtocol] Buscando treino para: userId=${userId}, dia=${diaDb}, ano=${ano}, semana=${semana} (protocolo: ${semanaProtocolo ? 'sim' : 'não'})`
      );
  ...
  static async executarSerie(userId, dadosExecucao) {
    try {
      ...
      await this.verificarAtualizacao1RM(
        userId,
        exercicio_id,
        peso_utilizado,
        repeticoes_realizadas
  ...
  static async finalizarExercicio(userId, exercicioId) {
    try {
      ...
      console.log(
        `[WorkoutProtocol] ✅ Exercício finalizado: ${totalSeries} séries, peso médio: ${pesoMedio?.toFixed(1)}kg`
      );
  ...
  static async finalizarTreino(userId, dadosTreino) {
    try {
      ...
      const { exercicios_realizados, tempo_total, observacoes } = dadosTreino;
      ...
      const pesoTotal = exercicios_realizados.reduce(
        (sum, ex) => sum + ex.peso_medio * ex.series_realizadas,
        0
      );
  ...
  static async verificarAtualizacao1RM(userId, exercicioId, peso, reps) {
    try {
      ...
      if (!rmAtualData || novo1RM > rmAtualData.rm_calculado + 2.5) {
        ...
        console.log(
          `[WorkoutProtocol] ✅ 1RM atualizado: ${novo1RM}kg (anterior: ${rmAtualData?.rm_calculado || 'N/A'}kg)`
        );
      }
    } catch (error) {
  ...
  static async verificarProgressaoSemanal(userId) {
    try {
      ...
      const treinosConcluidos =
        planejamentoSemana?.filter((p) => p.concluido && p.tipo_atividade === 'Treino').length || 0;
  ...
  static async verificarEAtualizarSemanaPorPlanejamento(userId) {
    try {
      ...
      if (planejamentoFuturo && planejamentoFuturo.length > 0) {
        console.log(
          `[WorkoutProtocol] 📅 Planejamento futuro detectado para semana ${proximaSemana}`
        );
        ...
        console.log(
          `[WorkoutProtocol] ✅ Semana atualizada para ${proximaSemana} devido ao planejamento futuro`
        );
        return true;
      }
