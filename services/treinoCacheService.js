...
export class TreinoCacheService {
  ...
  static verificarUltimoExercicio() {
    try {
      ...
      const exerciciosRestantes = [...exerciciosPlanejados].filter(
        (id) => !exerciciosExecutados.has(id)
      );
      ...
      return {
        ...
        motivo: eh_ultimo
          ? 'Todos exercícios planejados concluídos'
          : `${exerciciosRestantes.length} exercícios restantes`,
      };
    } catch (error) {
  ...
  static async commitarTreinoParaBanco() {
    try {
      ...
      if (planejamentoError) {
        console.warn(
          '[TreinoCacheService] Erro ao atualizar planejamento (não crítico):',
          planejamentoError
        );
      }
  ...
  static calcularEstatisticasSessao(sessao) {
    const exerciciosUnicos = new Set(sessao.execucoes.map((e) => e.exercicio_id)).size;
    const totalSeries = sessao.execucoes.length;
    const pesoTotal = sessao.execucoes.reduce(
      (total, exec) => total + (exec.peso_utilizado || 0) * (exec.repeticoes || 0),
      0
    );
  ...
  static validateState(state, options = {}) {
    ...
    if (
      !state.currentWorkout.exercicios ||
      !Array.isArray(state.currentWorkout.exercicios) ||
      state.currentWorkout.exercicios.length === 0
    ) {
    ...
    if (requireExecutions) {
      ...
      const isNewState =
        ...
        Date.now() - new Date(state.metadata.savedAt).getTime() < 5 * 60 * 1000;
      const hasExecutions = state.exerciciosExecutados.length > 0;

      if (!hasExecutions && !isNewState) {
        console.warn(
          '[TreinoCacheService] Nenhuma execução encontrada e não é estado novo - inválido'
        );
        return false;
      }
    }
  ...
  static async migrateOldData() {
    const migrations = ['old_workout_state', 'workoutProgress', 'workoutLocalBackup'];
