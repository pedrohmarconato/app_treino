...
class WeeklyPlanService {
  ...
  static async savePlan(userId, plan) {
    ...
    try {
      ...
      if (protocolosAtivos && protocolosAtivos.length > 0) {
        ...
        console.log(
          '[WeeklyPlanService.savePlan] 📅 Salvando com semana ANO:',
          semana,
          '| Semana PROTOCOLO:',
          semana_treino
        );
        ...
        console.warn(
          '[WeeklyPlanService.savePlan] ⚠️ Protocolo não encontrado, usando semana_treino = 1'
        );
      }
    } catch (error) {
    ...
    try {
      ...
      console.log('[WeeklyPlanService.savePlan] 🗑️ Deletando plano anterior para:', {
        userId,
        ano,
        semana,
      });
      ...
      console.log(
        '[WeeklyPlanService.savePlan] 📄 Registros completos:',
  ...
  static async getPlan(userId, useCache = true, ano = null, semana = null) {
    ...
    if (!ano || !semana) {
      ...
      if (semanaAtiva && semanaAtiva.semana_treino) {
        ...
        console.log(
          '[WeeklyPlanService.getPlan] ⚠️ Sem protocolo ativo, usando semana do ANO como fallback:',
          { ano, semana }
        );
      }
    ...
    console.log('[WeeklyPlanService.getPlan] 📅 Buscando plano para semana_treino:', {
      ano,
      semana,
    });
    ...
    try {
      ...
      console.log(
        '[WeeklyPlanService.getPlan] 🔄 Buscando dados do planejamento_semanal usando semana_treino...'
      );
  ...
  static async needsPlanning(userId) {
    try {
      ...
      try {
        ...
        console.warn(
          '[needsPlanning] View v_status_semanas_usuario indisponível, usando fallback:',
      ...
      if (error) {
        ...
        console.log(
          '[needsPlanning] Fallback - plano existente:',
          !!plan,
          'precisa:',
          needsPlanning
        );
  ...
  static async updateDay(userId, dia, config) {
    console.log('[editWeeklyPlan] Parâmetros recebidos:', {
      userId,
      diaIndex: dia,
      novoTreino: config,
    });
  ...
  static async getTodaysWorkout(userId) {
    ...
    console.log('[getTodaysWorkout] 🔄 NOVA LÓGICA: Buscando treino da semana do protocolo:', {
      userId,
      hoje: diaDb,
    });
  ...
  static async getTodaysWorkoutWithWeights(userId) {
    ...
    const exerciciosComPesos = await Promise.all(
      treinoDoDia.exercicios.map(async (exercicio) => {
        const { data: pesos } = await carregarPesosSugeridos(userId, exercicio.protocolo_treino_id);
  ...
  static async getWorkoutsWithWeeklyPlan(userId) {
    console.warn(
      '[getWorkoutsWithWeeklyPlan] ⚠️ Esta função não se aplica à estrutura atual do banco'
    );
...
export async function verificarDisponibilidadeCalendario() {
  try {
    console.log(
      '[verificarDisponibilidadeCalendario] Verificando disponibilidade da tabela "d_calendario"...'
    );
    ...
    if (
      ...
      console.log(
        '[verificarDisponibilidadeCalendario] ❌ Tabela "d_calendario" não existe no banco (404)'
      );
      return {
        disponivel: false,
    ...
    if (error) {
      console.log(
        '[verificarDisponibilidadeCalendario] ❌ Erro ao acessar "d_calendario":',
        error.message
      );
      return {
        disponivel: false,
    ...
    if (!data || data.length === 0) {
      console.log(
        '[verificarDisponibilidadeCalendario] ⚠️ Tabela "d_calendario" existe mas está vazia'
      );
      return {
        disponivel: true,
        populado: false,
    ...
    console.log(
      '[verificarDisponibilidadeCalendario] ❌ Erro crítico ao verificar:',
      error.message
    );
    return {
      disponivel: false,
  ...
  try {
    ...
    try {
      ...
      if (!resultado.success && resultado.reason === 'network_error') {
        console.warn(
          '[obterSemanaAtivaUsuario] 🌐 Erro de rede no calendário, continuando com dados em cache...'
        );
    ...
    const protocoloAtivo =
      protocolosAtivos && protocolosAtivos.length > 0 ? protocolosAtivos[0] : null;

    if (!protocoloAtivo) {
      console.log(
        '[obterSemanaAtivaUsuario] ❌ Nenhum protocolo ativo encontrado para usuário:',
        userId
      );
      return null;
    }
    ...
    try {
      ...
      console.log(
        '[obterSemanaAtivaUsuario] ⚠️ Não foi possível obter dados do calendário:',
        calendarioError.message
      );
    }
...
export async function carregarStatusSemanas(userId) {
  try {
    ...
    if (calendarioStatus.disponivel && calendarioStatus.temSemanaAtiva) {
      ...
      console.warn(
        '[carregarStatusSemanas] d_calendario não disponível ou sem semanas ativas. Retornando lista vazia.'
      );
      return [];
    }
  } catch (error) {
...
export async function buscarExerciciosTreinoDia(userId, diaAtual = null, semanaOverride = null) {
  try {
    console.log(
      '[buscarExerciciosTreinoDia] 🏋️‍♂️ Buscando exercícios do treino do dia para usuário:',
      userId,
      'semanaOverride:',
      semanaOverride
    );
    ...
    if (semanaOverride !== null) {
      ...
      if (!semanaAtiva) {
        console.log(
          '[buscarExerciciosTreinoDia] ❌ Não foi possível obter semana ativa do usuário'
        );
    ...
    console.log(
      '[buscarExerciciosTreinoDia] 🔄 NOVA LÓGICA: Usando semana do protocolo:',
      numeroSemana
    );
    ...
    if (planejamentoError || !planejamento) {
      console.log(
        '[buscarExerciciosTreinoDia] ❌ Nenhum planejamento encontrado para hoje:',
        planejamentoError?.message
      );
    ...
    if (planoError || !usuarioPlano) {
      console.log(
        '[buscarExerciciosTreinoDia] ❌ Plano do usuário não encontrado:',
        planoError?.message
      );
    ...
    console.log(
      '[buscarExerciciosTreinoDia] 📅 Semana de referência (protocolo):',
      semanaReferencia
    );
    console.log(
      '[buscarExerciciosTreinoDia] 🔍 DEBUG - Data formatada para consulta:',
      toSaoPauloDateString(hoje)
    );
    ...
    if (protocoloError || !protocoloTreinos?.length) {
      console.log(
        '[buscarExerciciosTreinoDia] ❌ Protocolo não encontrado:',
    ...
    console.log(
      '[buscarExerciciosTreinoDia] 📋 Protocolo encontrado:',
      protocoloTreinos.length,
      'exercícios'
    );
    ...
    if (exerciciosResponse.error) {
      console.error(
        '[buscarExerciciosTreinoDia] ❌ Erro ao buscar exercícios:',
        exerciciosResponse.error
      );
    ...
    console.log(
      '[buscarExerciciosTreinoDia] 🔍 DEBUG - Protocolos encontrados:',
      protocoloTreinos.map((p) => ({
    ...
    const exerciciosFormatados = protocoloTreinos.map((protocolo) => {
      ...
      const exercicioData = Array.isArray(protocolo.exercicios)
        ? protocolo.exercicios[0]
        : protocolo.exercicios;

      // Debug: verificar estrutura dos dados
      if (!exercicioData) {
        console.error(
          '[buscarExerciciosTreinoDia] ⚠️ exercicioData é undefined para protocolo:',
          protocolo
        );
        console.error(
          '[buscarExerciciosTreinoDia] Estrutura do protocolo:',
    ...
    console.log(
      '[buscarExerciciosTreinoDia] ✅ Exercícios formatados:',
      exerciciosFormatados.length
    );
...
async function carregarStatusSemanasComCalendario(userId) {
  try {
    ...
    const resultado = semanasCalendario.map((semanaCalendario) => {
      ...
      const planejamentosSemana =
        planejamentos?.filter(
          (p) => p.ano === semanaCalendario.ano && p.semana === semanaCalendario.semana_ano
        ) || [];
...
export async function verificarTreinoConcluido(userId) {
  try {
    console.log(
      '[verificarTreinoConcluido] 🔍 Verificando conclusão do treino de hoje para usuário:',
      userId
    );
    ...
    console.log('[verificarTreinoConcluido] 📅 Usando semana:', {
      ano,
      semana,
      diaSemana,
      fonte: semanaAtiva ? 'protocolo' : 'civil',
    });
...
export async function resetarTreinoHoje(userId, motivoReset = 'Reset manual') {
  try {
    ...
    console.log('[resetarTreinoHoje] 📅 Usando semana:', {
      ano,
      semana,
      diaSemana,
      fonte: semanaAtiva ? 'protocolo' : 'civil',
    ...
    const { data: resetPlan, error: planError } = await update(
      'planejamento_semanal',
      {
        concluido: false,
        data_conclusao: null,
