...
export class CalendarioService {
  ...
  static async atualizarSemanaAtual() {
    ...
    try {
      ...
      if (!dataAtualizada || dataAtualizada.length === 0) {
        console.warn(
          '[CalendarioService] ⚠️ Nenhum registro encontrado para a data atual:',
          dataHoje
        );
  ...
  static async verificarSemanaAtual() {
    ...
    try {
      ...
      if (result.offline) {
        console.warn(
          '[CalendarioService] 📵 Operação offline - usando dados em cache se disponível'
        );
        return {
          success: false,
  ...
  static async manterSemanaAtualizada() {
    ...
    if (!verificacao.success) {
      ...
      console.error(
        '[CalendarioService] ❌ Erro persistente na verificação, tentando reset forçado...'
      );
...
export const manterSemanaAtualizada =
  CalendarioService.manterSemanaAtualizada.bind(CalendarioService);
