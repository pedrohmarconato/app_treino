...
export class WorkoutCompletionService {
  ...
  static async diagnosticarEstadoTreino() {
    ...
    console.log(
      `  - Usuário: ${currentUser ? `${currentUser.nome} (ID: ${currentUser.id})` : 'NÃO ENCONTRADO'}`
    );
    console.log(
      `  - Tempo início: ${workoutStartTime ? new Date(workoutStartTime).toLocaleString() : 'NÃO DEFINIDO'}`
    );
    console.log(
      `  - Treino atual: ${currentWorkout ? currentWorkout.grupo_muscular || currentWorkout.tipo : 'NÃO DEFINIDO'}`
    );
    ...
    console.log('🔧 Funções:');
    console.log(
      `  - showNotification: ${typeof window.showNotification === 'function' ? 'OK' : 'AUSENTE'}`
    );
    console.log(
      `  - carregarDashboard: ${typeof window.carregarDashboard === 'function' ? 'OK' : 'AUSENTE'}`
    );
    console.log(
      `  - finalizarTreino: ${typeof window.finalizarTreino === 'function' ? 'OK' : 'AUSENTE'}`
    );
    ...
    if (workoutStartTime) {
      ...
      console.log(
        `⏱️ Duração do treino: ${duracaoMin} minutos (${Math.round(duracaoMs / 1000)} segundos)`
      );
    }
