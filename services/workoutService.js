...
export async function fetchExerciciosTreino(grupoMuscular, protocoloId) {
  const { data } = await query('protocolo_treinos', {
    select:
      'id, protocolo_id, exercicio_id, semana_referencia, dia_semana, percentual_1rm_base, percentual_1rm_min, percentual_1rm_max, series, repeticoes_alvo, tempo_descanso, ordem_exercicio, observacoes, exercicios!inner ( id, nome, grupo_muscular, equipamento, tempo_descanso_padrao )',
    eq: {
...
export async function carregarPesosSugeridos(userId, protocoloTreinoId) {
  try {
    console.log('[workoutService] carregarPesosSugeridos chamado com:', {
      userId,
      protocoloTreinoId,
    });
    ...
    const peso_base =
      rm1 > 0 ? Math.round((rm1 * (protocoloTreino.percentual_1rm_base || 75)) / 100) : 0;
    const peso_minimo =
      rm1 > 0 ? Math.round((rm1 * (protocoloTreino.percentual_1rm_min || 70)) / 100) : 0;
    const peso_maximo =
      rm1 > 0 ? Math.round((rm1 * (protocoloTreino.percentual_1rm_max || 80)) / 100) : 0;
...
function validarDadosExecucao(dados) {
  ...
  if (
    !dados.usuario_id ||
    (typeof dados.usuario_id !== 'string' && typeof dados.usuario_id !== 'number')
  ) {
...
async function salvarExecucoesEmLoteComRetry(execucoes, maxRetries = 3, tentativa = 1) {
  try {
    console.log(
      `[salvarExecucoesEmLote] Tentativa ${tentativa}/${maxRetries} - Salvando ${execucoes.length} execuções em lote`
    );
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      console.log(
        `[salvarExecucoesEmLote] Processando chunk ${i + 1}/${chunks.length} com ${chunk.length} itens`
      );
      ...
      try {
        ...
        if (error) {
          ...
          console.log(
            `[salvarExecucoesEmLote] Chunk ${i + 1} salvo com sucesso: ${dadosValidados.length} registros`
          );
        }
      } catch (networkError) {
    ...
    if (dadosNaoSalvos.length > 0 && tentativa < maxRetries) {
      ...
      const temErrosRecuperaveis = errosDetalhados.some(
        (erro) => erro.tipo === 'network' || erro.tipo === 'total'
      );

      if (temErrosRecuperaveis) {
        console.log(
          `[salvarExecucoesEmLote] ${dadosNaoSalvos.length} dados não salvos. Tentando novamente em ${tentativa * 2}s...`
        );
...
export async function marcarTreinoConcluido(userId) {
  try {
    ...
    const diaSemana = window.WeeklyPlanService
      ? window.WeeklyPlanService.dayToDb(hoje.getDay())
      : hoje.getDay(); // Converter para formato DB
