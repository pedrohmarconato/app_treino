...
export class TreinoMetricasService {
  ...
  static async calcularMetricasDia(userId, data) {
    try {
      ...
      const { data: metricas, error: errorMetricas } = await supabase.rpc('calc_metricas_dia', {
        p_usuario_id: userId,
        p_data: dataStr,
      });
  ...
  static async calcularMetricasManual(userId, data) {
    try {
      ...
      const metricas = {
        ...
        peso_medio:
          execucoes.reduce((sum, e) => sum + (e.peso_utilizado || 0), 0) / execucoes.length,
        tempo_estimado: Math.round(
          execucoes.reduce((sum, e) => sum + (e.tempo_estimado_serie_segundos || 0), 0) / 60
        ),
        grupos_musculares: [...new Set(execucoes.map((e) => e.grupo_muscular))],
        intensidade_media: this.calcularMedia(
          execucoes.map((e) => e.intensidade_percentual).filter(Boolean)
        ),
        prs_realizados: execucoes.filter((e) => e.qualidade_performance === 'Excelente').length,
        falhas: execucoes.filter((e) => e.falhou).length,
        taxa_sucesso:
          Math.round(
            ((execucoes.length - execucoes.filter((e) => e.falhou).length) / execucoes.length) *
              100 *
              10
          ) / 10,
      };
    ...
    try {
      ...
      const resumo = {
        ...
        dias: Object.values(porDia).map((dia) => ({
          ...
          tempo_estimado: Math.round(
            dia.treinos.reduce((sum, t) => sum + (t.tempo_estimado_serie_segundos || 0), 0) / 60
          ),
