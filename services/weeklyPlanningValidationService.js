...
class WeeklyPlanningValidationService {
  ...
  static canEditDay(diaIndex) {
    ...
    console.log('[canEditDay] Verificando dia:', {
      diaIndex,
      dayConfig,
      isPast: this.isDayInPast(diaIndex),
    });
  ...
  static async validateWeeklyPlan() {
    try {
      ...
      console.log('[WeeklyPlanningValidation] Iniciando validação...', {
      ...
      console.log('[WeeklyPlanningValidation] Usando semana do protocolo ativo:', {
        semana_treino,
        protocolo_id: planoAtivo[0].protocolo_treinamento_id,
      });
      ...
      const { data: planejamentos, error } = await query('planejamento_semanal', {
        select:
          'id, dia_semana, tipo_atividade, concluido, data_conclusao, ano, semana, semana_treino, usuario_id',
        eq: {
          usuario_id: currentUser.id,
          ano: ano,
      ...
      console.log(
        '[WeeklyPlanningValidation] Planejamentos encontrados:',
      ...
      for (let dia = 0; dia < diaAtual; dia++) {
        // Só verificar dias que já passaram
