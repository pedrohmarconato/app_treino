...
export class WeightCalculatorService {
   ...
   */
  static async calcularPesosExercicio(
    userId,
    exercicioId,
    semanaAtual,
    grupoMuscular,
    ...
    try {
      console.log(
        `[WeightCalculator] Calculando pesos: user=${userId}, exercicio=${exercicioId}, semana=${semanaAtual}`
      );
      ...
      if (!rm1 || rm1 <= 0) {
        console.log(
          `[WeightCalculator] 1RM não encontrado para exercício ${exercicioId}, usando valores padrão`
        );
      ...
      if (!exercicioId || !semanaAtual || !grupoMuscular || !protocoloId) {
        console.warn(`[WeightCalculator] Parâmetros inválidos:`, {
          exercicioId,
          semanaAtual,
          grupoMuscular,
          protocoloId,
      ...
      if (error || !protocoloTreino) {
        console.warn(`[WeightCalculator] Protocolo não encontrado:`, {
          exercicioId,
          semanaAtual,
          grupoMuscular,
          protocoloId,
  ...
  static async calcularPesosTreino(userId, tipoAtividade, protocoloId, semanaAtual) {
    try {
      ...
      if (!tipoAtividade || !protocoloId || !semanaAtual) {
        console.warn(`[WeightCalculator] Parâmetros inválidos para buscar treino:`, {
          tipoAtividade,
          protocoloId,
          semanaAtual,
        });
      ...
      if (error || !exerciciosTreino) {
        ...
        console.log(
          `[WeightCalculator] 🔍 Exercícios disponíveis para protocolo ${protocoloId}:`,
          protocoloEspecifico
        );
        ...
        console.log(
          `[WeightCalculator] 🔍 Treinos disponíveis para grupo muscular ${tipoAtividade}:`,
          treinoEspecifico
        );
        ...
        console.log(
          `[WeightCalculator] 🔍 Semanas disponíveis para protocolo ${protocoloId}, grupo ${tipoAtividade}:`,
          semanasDisponiveis?.map((s) => s.semana_referencia)
        );
        ...
        console.log(
          '🗂️ Todos os protocolo_id existentes:',
          todosProtocolos?.map((p) => p.protocolo_id)
        );
        ...
        console.log(
          `🏋️ Todos os grupos musculares disponíveis:`,
          todosGrupos?.map((g) => g.grupo_muscular)
        );
        ...
        console.log(
          `📅 Todas as semanas para protocolo ${protocoloId}, grupo ${tipoAtividade}:`,
          todasSemanas?.map((s) => s.semana_referencia)
        );
        ...
        console.log(
          `🎯 Exercícios para protocolo ${protocoloId}, grupo ${tipoAtividade} (TODAS as semanas):`,
          combinacaoProxima
        );
        ...
        throw new Error(
          `Nenhum exercício encontrado para: grupo_muscular=${tipoAtividade}, protocolo_id=${protocoloId}, semana=${semanaAtual}`
        );
      }
      ...
      console.log(
        `[WeightCalculator] ✅ ${exerciciosComPesos.length} exercícios com pesos calculados`
      );
