...
class StorageMonitor {
    ...
    checkStorageQuota() {
        ...
        return {
            totalBytes,
            totalMB,
            status: totalMB >= this.CRITICAL_THRESHOLD_MB ? 'critical' : 
                   totalMB >= this.WARNING_THRESHOLD_MB ? 'warning' : 'ok',
            analysis
        };
    }
    ...
    analyzeStorageUsage() {
        ...
        const categories = {
            ...
            other: items.filter(i => 
                !i.key.includes('workout') && !i.key.includes('treino') &&
                !i.key.includes('analytics') && !i.key.includes('events') &&
                !i.key.includes('cache') && !i.key.includes('temp') &&
                !i.key.includes('sync') && !i.key.includes('pending')
            )
        };
