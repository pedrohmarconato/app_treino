...
async function inicializarPlanejamento(usuarioId, modoEdicaoParam = false) {
  ...
  try {
    ...
    if (!semanaAtiva) {
      showNotification(
        'Nenhuma semana ativa encontrada. Configure o calendário primeiro.',
        'warning'
      );
      return;
    }
    ...
    if (jaProgramada && planoExistente) {
      console.log(
        '[inicializarPlanejamento] Semana já programada - Modo visualização/edição limitada'
      );
      ...
      console.log(
        '[inicializarPlanejamento] Inconsistência: marcada como programada mas sem plano'
      );
    ...
    if (planoExistente && modoEdicao) {
      ...
      console.log(
        '[inicializarPlanejamento] Plano existente encontrado, carregando para visualização/edição...'
      );
      planejamentoAtual = planoExistente;
    } else {
    ...
    const modal =
      document.getElementById('modalPlanejamento') || document.getElementById('modal-planejamento');
...
async function finalizarPlanejamentoExistente() {
  try {
    ...
    console.warn(
      '[finalizarPlanejamentoExistente] Plano existente inválido, continuando com planejamento'
    );
  } catch (error) {
...
async function atualizarStatusVisuaisDias(usuarioId, semanaAtiva) {
  ...
  if (!modal) {
    ...
    console.log(
      '[atualizarStatusVisuaisDias] Modais encontrados no DOM:',
      modals.length,
      Array.from(modals).map((m) => m.id)
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      '[atualizarStatusVisuaisDias] Planejamentos encontrados:',
    ...
    console.log('[atualizarStatusVisuaisDias] Dia atual:', {
      diaAtualJS,
      diaAtualTemplate,
      diaNome: ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sab'][diaAtualJS],
    });

    // Array de dias na ordem do template
    const diasOrdemTemplate = [
      'Segunda',
      'Terça',
      'Quarta',
      'Quinta',
...
function adicionarIndicadorModoEdicao() {
  const container =
    document.querySelector('.planning-page-container') ||
...
function validarPlanejamento() {
  ...
  if (diasPreenchidos === 0) {
    ...
    messages.push(
      `⚠️ Recomendado preencher todos os 7 dias (faltam ${7 - diasPreenchidos}). Você pode salvar assim mesmo.`
    );
  ...
  const temAlgumTreino = Object.values(planejamentoAtual).some(
    (treino) => treino && (treino.categoria === 'muscular' || treino.categoria === 'cardio')
  );
  ...
  console.log('[validarPlanejamento] Validação:', {
    modoEdicao,
    isValid,
    diasPreenchidos,
    messages,
...
function atualizarEstatisticasPlanejamento(diasPreenchidos) {
  try {
    ...
    if (workoutCountEl) {
      const totalTreinos = Object.values(planejamentoAtual).filter(
        (treino) =>
          treino &&
          (treino.categoria === 'muscular' ||
            treino.categoria === 'cardio' ||
            treino.categoria === 'treino')
      ).length;
...
window.abrirSeletorTreino = async function (dia, nomeDia) {
  ...
  if (!title || !options) {
    console.error('[abrirSeletorTreino] Elementos internos não encontrados!', {
      title: !!title,
      options: !!options,
    });
    return;
...
function criarOpcaoTreino(treino, diaDestino) {
  ...
  if (treino.categoria === 'muscular') {
    const diasComEsseTreino = Object.keys(planejamentoAtual).filter(
      (dia) => planejamentoAtual[dia] && planejamentoAtual[dia].tipo === treino.tipo
    );
...
async function selecionarTreinoParaDia(treino, dia) {
  ...
  console.log(
    '[selecionarTreinoParaDia] Object.keys(planejamentoAtual):',
    Object.keys(planejamentoAtual)
  );
  console.log(
    '[selecionarTreinoParaDia] Total de dias planejados:',
    Object.keys(planejamentoAtual).length
  );
  ...
  if (dayContent) {
    ...
    addButton.addEventListener('click', (e) => {
      ...
      if (window.abrirSeletorTreino) {
        ...
        console.error(
          '[removerTreinoDoDia] A função abrirSeletorTreino não foi encontrada no window.'
        );
      }
    });
...
async function salvarPlanejamentoSemanal() {
  ...
  try {
    ...
    console.log(
      '[salvarPlanejamentoSemanal] Object.keys(planejamentoAtual):',
      Object.keys(planejamentoAtual)
    );
    console.log(
      '[salvarPlanejamentoSemanal] Object.entries(planejamentoAtual):',
      Object.entries(planejamentoAtual)
    );
    ...
    for (let dia = 0; dia < 7; dia++) {
      ...
      if (dia === 0) {
        ...
        console.log(
          '[salvarPlanejamentoSemanal] - planejamentoAtual["0"]:',
          planejamentoAtual['0']
        );
        console.log(
          '[salvarPlanejamentoSemanal] - planejamentoAtual.domingo:',
          planejamentoAtual.domingo
        );
        console.log(
          '[salvarPlanejamentoSemanal] - planejamentoAtual["domingo"]:',
          planejamentoAtual['domingo']
      ...
      if (planejamentoAtual[dia] || planejamentoAtual[String(dia)]) {
        ...
        console.log(
          `[salvarPlanejamentoSemanal] 🎯 DIA ${dia} - Encontrado por índice direto:`,
          treino
        );
      } else {
        ...
        for (const [diaKey, t] of Object.entries(planejamentoAtual)) {
          if (diasMap[diaKey] === dia) {
            treino = t;
            console.log(
              `[salvarPlanejamentoSemanal] 🎯 DIA ${dia} - Encontrado via mapeamento (${diaKey}):`,
              treino
            );
            break;
          }
        ...
        if (!numeroTreino) {
          ...
          if (protocoloTreinos && protocoloTreinos.length > 0) {
            const treinosDisponiveis = [
              ...new Set(protocoloTreinos.map((pt) => pt.semana_referencia)),
            ];
    ...
    if (Object.keys(planejamentoParaSupabase).length === 0) {
      ...
      console.error(
        '[salvarPlanejamentoSemanal] planejamentoAtual estava vazio:',
        planejamentoAtual
      );
    ...
    console.log(
      '[salvarPlanejamentoSemanal] 🚀 OBJETO COMPLETO PARA SUPABASE:',
      planejamentoParaSupabase
    );
    console.log(
      '[salvarPlanejamentoSemanal] 📊 QUANTIDADE DE DIAS A SALVAR:',
      Object.keys(planejamentoParaSupabase).length
    );
    ...
    console.log(
      '[salvarPlanejamentoSemanal] 📋 Dados completos:',
      JSON.stringify(planejamentoParaSupabase, null, 2)
    );
    ...
    if (semanaAtiva) {
      const marcacaoResult = await marcarSemanaProgramada(
        userId,
        semanaAtiva.semana_treino,
        userId
      );
...
function fecharModalPlanejamento() {
  ...
  console.log(
    '[fecharModalPlanejamento] Verificando window.mostrarTela:',
...
async function abrirCriacaoPlanejamento(usuarioId) {
  ...
  try {
    ...
    if (modal) {
      ...
      if (content) {
        console.log(
          '[abrirCriacaoPlanejamento] Dimensões do conteúdo:',
      ...
      console.log(
        '[abrirCriacaoPlanejamento] Elementos disponíveis:',
...
async function abrirEdicaoPlanejamento(usuarioId) {
  ...
  try {
    ...
    if (!planoExistente) {
      showNotification(
        'Não há plano semanal ativo para editar. Crie um planejamento primeiro.',
        'warning'
      );
      return;
    }
    ...
    if (!temDiasEditaveis) {
      showNotification(
        'Todos os treinos da semana já foram realizados. Não há nada para editar.',
        'info'
      );
      return;
    }
    ...
    if (validationResult.hasIssues && validationResult.needsAttention) {
      console.log(
        '[abrirEdicaoPlanejamento] Treinos perdidos detectados, mostrando modal de ajuste...'
      );
    ...
    if (error.message && error.message.includes('Failed to fetch')) {
      ...
      showNotification(
        'Erro ao carregar planejamento para edição: ' + (error.message || error),
        'error'
      );
    }
  }
...
window.salvarPlanejamento = async function () {
  ...
  const botaoSalvar = document.querySelector(
    '.btn-save-plan, [onclick*="salvarPlanejamento"], button[onclick*="salvarPlanejamento"]'
  );
  if (botaoSalvar) {
...
window.corrigirEstadoPlanejamento = function () {
  ...
  const chavesParaRemover = Object.keys(planejamentoAtual).filter(
    (chave) => !chavesValidas.includes(chave) && !chavesValidas.includes(parseInt(chave))
  );
  ...
  Object.entries(tiposUsados).forEach(([tipo, dias]) => {
    if (dias.length > 1) {
      console.log(
        `[corrigirEstadoPlanejamento] ⚠️ Duplicata encontrada para ${tipo} nos dias: ${dias.join(', ')}`
      );
      // Manter apenas o primeiro dia, remover os outros
      for (let i = 1; i < dias.length; i++) {
        console.log(
          `[corrigirEstadoPlanejamento] Removendo duplicata de ${tipo} do dia ${dias[i]}`
        );
  ...
  console.log(
    '[corrigirEstadoPlanejamento] ✅ CORREÇÃO CONCLUÍDA! ${correcoesFeiras} problemas corrigidos'
  );
  console.log('[corrigirEstadoPlanejamento] Estado final:', planejamentoAtual);

  if (window.showNotification) {
    window.showNotification(
      `✅ Estado corrigido! ${correcoesFeiras} problemas resolvidos`,
      'success'
    );
  }
...
window.diagnosticarProblemaSemanea = async function () {
  ...
  try {
    ...
    if (semanaUsuario) {
      ...
      console.log(
        '[diagnosticarProblemaSemanea] 📋 Planejamento semana usuário:',
        planejamentoAtual
      );
    }
    ...
    if (semanaUsuario !== semanaCalendario) {
      console.log(
        '[diagnosticarProblemaSemanea] ✅ DIFERENÇA NORMAL: Cada usuário tem sua própria progressão'
      );

      if (window.showNotification) {
        window.showNotification(
          `✅ Sistema atualizado: Usuário está na semana ${semanaUsuario} do seu protocolo`,
          'info'
        );
      }
...
window.corrigirProblemaSemanea = async function () {
  ...
  try {
    ...
    console.log(
      '[corrigirProblemaSemanea] 📝 Use a progressão individual do usuário em usuario_plano_treino.semana_atual'
    );
...
window.diagnosticoCompletoSalvamento = async function () {
  ...
  try {
    ...
    if (
      resultados.usuario.success &&
      resultados.protocoloAtivo.success &&
      resultados.acessoTabela.success
    ) {
