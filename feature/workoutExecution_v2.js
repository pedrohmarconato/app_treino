// VERSÃO 2.0 - (2025-06-17)
// Cache totalmente migrado para TreinoCacheService
// Arquivo seguro para deploy imediato
...
class WorkoutExecutionManager {
    ...
    async iniciarTreino() {
        try {
            ...
            try {
                ...
                const disposicao = await DisposicaoInicioModal.solicitar();
                ...
                if (disposicao !== null) {
                    ...
                    await DisposicaoInicioModal.salvarValor(disposicao);
    ...
    async confirmarSerie(exercicioId, serieNumero) {
        try {
            ...
            this.marcarSerieComoConcluida(serieElement);
            
            // Iniciar timer de descanso se não for a última série
            if (serieNumero < (exercicio.series - 1)) {
                this.iniciarTimerDescanso(exercicio.tempo_descanso || 60);
            }
            
            // Verificar se exercício está completo
            this.verificarExercicioCompleto(exercicioId);
            
        } catch (error) {
            console.error('[WorkoutExecution] Erro ao confirmar série:', error);
            window.showNotification && window.showNotification('Erro ao registrar série', 'error');
        }
    }

    // Marcar série como concluída
    marcarSerieComoConcluida(seriesItem) {
        seriesItem.classList.add('completed');
        seriesItem.style.backgroundColor = 'rgba(168, 255, 0, 0.1)';
        seriesItem.style.border = '1px solid var(--accent-green, #a8ff00)';
        
        // Atualizar conteúdo
        const inputs = seriesItem.querySelector('.series-inputs');
        inputs.innerHTML = `
            <div style="flex: 1; color: var(--text-primary, #fff); font-weight: 600;">
                ${seriesItem.querySelector('.peso-input').value}kg × ${seriesItem.querySelector('.reps-input').value} reps
            </div>
        `;
                        ...
                        ${Array.from({length: series}, (_, i) => `
                            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 8px; padding: 12px; background: #1a1a1a; border-radius: 8px;">
                                ...
                                <button onclick="workoutExecutionManager.confirmarSerieEmergencia(${index}, ${i})" 
                                        style="background: #13f1fc; color: #000; border: none; padding: 8px 12px; border-radius: 4px; cursor: pointer; font-weight: bold;">✓</button>
                            </div>
                        `).join('')}
                    </div>
                </div>
    ...
    async restoreState() {
        const state = await this.persistence.restoreState();
        if (state) {
...
// ===== FUNÇÕES OBSOLETAS - MANTER TEMPORARIAMENTE =====
/**
 * @deprecated [v2.0] - Use TreinoCacheService.saveWorkoutState()
 * Será removido em 2025-07-01
 */

/**
 * @deprecated [v2.0] - Use TreinoCacheService.getWorkoutState()
 * Será removido em 2025-07-01
 */

/**
 * @deprecated [v2.0] - Use TreinoCacheService.limparCache()
 * Será removido em 2025-07-01
 */

// ===== FIM DAS FUNÇÕES OBSOLETAS =====
