...
class AppStateManager {
  ...
  set(key, value) {
    ...
    if (key === 'workoutStartTime' && value !== null && value !== undefined) {
      ...
      if (isNaN(numValue) || numValue <= 0) {
        console.error(
          '[AppState] Tentativa de definir workoutStartTime inválido:',
          value,
          'tipo:',
          typeof value
        );
  ...
  get currentWorkout() {
    return this.state.currentWorkout;
  }
  get currentExercises() {
    return this.state.currentExercises;
  }
  get currentExerciseIndex() {
    return this.state.currentExerciseIndex;
  }
  get weekPlan() {
    return this.state.weekPlan;
  }
  get users() {
  ...
  get weekNavigation() {
    return this.state.weekNavigation;
  }
  get currentWeek() {
    return this.state.weekNavigation.currentWeek;
  }
  get viewingWeek() {
    return this.state.weekNavigation.viewingWeek;
  }
  get isViewingCurrentWeek() {
    return this.state.weekNavigation.isViewingCurrent;
  }
  ...
  setViewingWeek(ano, semana) {
    const isCurrentWeek =
      this.state.weekNavigation.currentWeek &&
