...
class ModalManager {
  ...
  show(id) {
    ...
    if (this.getVisibleModalsCount() >= this.config.maxConcurrentModals) {
      console.warn(
        `[ModalManager] Limite de modais simultâneos atingido (${this.config.maxConcurrentModals})`
      );
      return false;
    }
  ...
  getVisibleModalsCount() {
    return Array.from(this.activeModals.values()).filter((config) => config.isVisible).length;
  }
...
export const registerModal = (id, instance, options) =>
  modalManager.register(id, instance, options);
