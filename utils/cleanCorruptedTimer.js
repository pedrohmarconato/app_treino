...
export function cleanCorruptedTimerData() {
    ...
    if (tempoSalvo) {
        try {
            ...
            const tempoValido = typeof dados.tempo === 'number' && !isNaN(dados.tempo) && dados.tempo >= 0;
            const timestampValido = typeof dados.ultimaAtualizacao === 'number' && !isNaN(dados.ultimaAtualizacao) && dados.ultimaAtualizacao > 0;
            const tempoMuitoAntigo = dados.ultimaAtualizacao && (Date.now() - dados.ultimaAtualizacao > 24 * 60 * 60 * 1000); // 24h
        ...
        try {
            ...
            if (estado.cronometro && estado.cronometro.workoutStartTime) {
                ...
                if (isNaN(startTime) || startTime <= 0) {
                    console.warn('[cleanCorruptedTimer] ⚠️ workoutStartTime corrompido:', estado.cronometro.workoutStartTime);
            ...
            if (estado.cronometro) {
                ['tempoAtual', 'tempoUltimaAtualizacao'].forEach(campo => {
                    if (estado.cronometro[campo] && isNaN(Number(estado.cronometro[campo]))) {
                        console.warn(`[cleanCorruptedTimer] ⚠️ Campo ${campo} corrompido:`, estado.cronometro[campo]);
...
export function startTimerMonitoring() {
    ...
    const monitoringInterval = setInterval(() => {
        ...
        if (problemas > 0) {
            console.warn(`[cleanCorruptedTimer] ⚠️ ${problemas} problemas detectados durante monitoramento`);
        }
    }, 5 * 60 * 1000); // 5 minutos
