...
export class AvaliacaoTreinoComponent {
  ...
  static configurarEventListeners() {
    ...
    modal.querySelectorAll('.likert-option').forEach((option) => {
      option.addEventListener('click', () => {
        ...
        modal.querySelectorAll('.likert-option').forEach((opt) => opt.classList.remove('selected'));
        ...
        console.log(
          '[AvaliacaoTreino] Fadiga pós-treino selecionada:',
          avaliacaoEstado.post_workout
        );
      });
    });
  ...
  static mostrarConfirmacaoSaida() {
    if (
      confirm('⚠️ Tem certeza que quer sair sem avaliar?\n\nSeus dados do treino serão perdidos!')
    ) {
      // Marcar como falta
...
console.log(
  '[AvaliacaoTreino] ✅ Componente registrado globalmente: window.AvaliacaoTreinoComponent'
);
