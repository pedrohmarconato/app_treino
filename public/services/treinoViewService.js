...
export class TreinoViewService {
        ...
        try {
            ...
            const stats = {
                ...
                volume: {
                    ...
                    medio_por_treino: treinos.reduce((sum, t) => sum + (t.volume_total_dia || 0), 0) / 
                ...
                performance: {
                    ...
                    taxa_sucesso: ((treinos.length - treinos.filter(t => t.falhou).length) / treinos.length * 100).toFixed(1)
                },
    ...
    static temMultiplosGruposRelevantes(treinos) {
        ...
        const estatisticasPorGrupo = grupos.map(grupo => ({
            ...
            exercicios: [...new Set(treinos.filter(t => t.grupo_muscular === grupo).map(t => t.exercicio_id))].length
        }));
        
        // Filtrar grupos com pelo menos 2 séries OU 2 exercícios diferentes
        const gruposRelevantes = estatisticasPorGrupo.filter(stat => 
            stat.series >= 2 || stat.exercicios >= 2
        );
    ...
    static agruparPorExercicio(treinos) {
        ...
        Object.values(grupos).forEach(grupo => {
            ...
            grupo.metricas.intensidade_media = intensidades.length > 0 ? 
                intensidades.reduce((a, b) => a + b, 0) / intensidades.length : 0;
        });
            ...
            if (!grupos[data]) {
                grupos[data] = {
                    ...
                    metricas: {
                        ...
                        tempo_estimado: Math.round(treino.tempo_estimado_serie_segundos * treino.total_series_dia / 60)
                    }
                };
            }
