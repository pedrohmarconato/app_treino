...
(function() {
    ...
    if (window.workoutExecutionManager) {
        ...
        window.workoutExecutionManager.navegarParaTelaWorkout = async function() {
            ...
            try {
                ...
                const isVisible = workoutScreen && window.getComputedStyle(workoutScreen).display !== 'none';
    ...
    setTimeout(() => {
        ...
        if (button) {
            ...
            button.addEventListener('click', async function(e) {
                if (this.getAttribute('data-state') === 'resume' && this.getAttribute('data-action') === 'resumeWorkout') {
                    ...
                    try {
                        ...
                        // Tentar resumir
                        if (window.workoutExecutionManager && window.workoutExecutionManager.resumeFromCache) {
            ...
            }, true); // Usar captura para garantir que seja executado primeiro
        }
    }, 2000);
    ...
    window.debugRecoveryNavigation = async function() {
        ...
        if (checks.workoutScreen) {
            checks.workoutScreenVisible = window.getComputedStyle(checks.workoutScreen).display !== 'none';
        }
