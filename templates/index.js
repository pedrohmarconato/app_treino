...
const _modalPlanejamentoTemplate = safe(
  modalPlanejamentoTemplate,
  () => '<div>Modal planejamento não disponível</div>'
);
const _modalPlanejamentoStyles = safe(modalPlanejamentoStyles, '');
const _planejamentoSemanalPageTemplate = safe(
  planejamentoSemanalPageTemplate,
  () => '<div>Planejamento semanal não disponível</div>'
);
  ...
  try {
    ...
    switch (templateName) {
        ...
        const attachLoginHandlers = () => {
          try {
            ...
            if (forgotBtn) {
              ...
              freshForgotBtn.addEventListener(
                'click',
                async (e) => {
                  try {
                    e.preventDefault();
                    ...
                    if (typeof window.abrirRecuperacaoSenha === 'function') {
                      ...
                      const { default: ForgotPasswordModal } = await import(
                      ...
                      if (result?.success && window.showNotification) {
                        window.showNotification(
                          `Email enviado para ${result.email}! Verifique sua caixa de entrada.`,
                          'success'
                        );
                      }
                    }
            ...
            if (signupBtn) {
              ...
              freshSignupBtn.addEventListener(
                'click',
                async (e) => {
                  try {
                    e.preventDefault();
                    ...
                    if (typeof window.abrirModalCadastro === 'function') {
                      ...
                      // Fallback: importar modal diretamente
                      const { default: CadastroUsuarioModal } = await import(
                        '../components/CadastroUsuarioModal.js'
                      );
        ...
        if (loginScreen) {
          ...
          setTimeout(() => {
            ...
            if (loginScreenDelayed) {
              ...
              console.log(
                '[renderTemplate] ✅ Classe "active" adicionada à tela login (com delay)'
              );
      ...
      case 'home':
        ...
        try {
          ...
          if (homeScreen) {
            ...
            console.log(
              '[renderTemplate] 🔍 Elementos disponíveis:',
            ...
            setTimeout(() => {
              ...
              if (homeScreenDelayed) {
                ...
                console.log(
                  '[renderTemplate] ✅ Classe "active" adicionada à tela home (com delay)'
                );
              }
            }, 10);
          }
        ...
        if (workoutScreen) {
          ...
          setTimeout(() => {
            ...
            if (workoutScreenDelayed) {
              ...
              console.log(
                '[renderTemplate] ✅ Classe "active" adicionada à tela workout (com delay)'
              );
            }
          }, 10);
        }
      ...
      case 'planejamentoSemanalPage': {
        ...
        console.log('[renderTemplate] Container:', containerEl);
        console.log(
          '[renderTemplate] planejamentoSemanalPageTemplate disponível:',
          !!window.planejamentoSemanalPageTemplate
        );
        ...
        console.log(
          '[renderTemplate] HTML gerado para planejamento, tamanho:',
          planejamentoHTML.length
        );
    ...
    if (templateName !== 'login') {
      ...
      if (!document.getElementById('login-screen')) {
        ...
        console.warn(
          '[renderTemplate] (timeout fallback) Login já está presente no DOM, não renderizando novamente.'
        );
      }
    }
  }
...
async function initializeHomeComponents() {
  ...
  try {
    ...
    console.log(
      '[initializeHomeComponents] ✅ Usuário encontrado:',
      currentUser.nome,
      'ID:',
      currentUser.id
    );
...
function setupContextualWorkoutButton() {
  try {
    ...
    if (!contextualBtn) {
      console.warn(
        '[setupContextualWorkoutButton] Nenhum botão contextual encontrado (procurou #contextual-workout-btn e #start-workout-btn)'
      );
      return;
    }
...
function setupHomeAnimations() {
  try {
    ...
    const sections = document.querySelectorAll(
      '.training-plan, .metrics-section, .weekly-plan-section'
    );
